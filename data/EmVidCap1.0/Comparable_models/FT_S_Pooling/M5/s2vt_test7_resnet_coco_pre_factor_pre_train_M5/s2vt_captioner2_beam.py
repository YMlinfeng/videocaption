...
def predict_single_word(net, pad_img_feature, previous_word, output1='probs', output2='probs_senti'):
  ...
  output_preds = 0.8*output_preds1 + 0.2*output_preds2       ###tpj
  return output_preds
...
def sample_captions(net, image_features,
  ...
  while num_done < batch_size:
    ...
    net_output_probs = 0.8*net_output_probs1 + 0.2*net_output_probs2           ###tpj
    for index in range(batch_size):
...
def main():
  ...
  #DIR = './snapshots'
  DIR = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream3/models'
  #VOCAB_FILE = './yt_coco_mvad_mpiimd_vocabulary.txt'
  VOCAB_FILE = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/vocabulary_refine.txt'
  #FRAMEFEAT_FILE_PATTERN = './yt_allframes_vgg_fc7_{0}.txt'
  FRAMEFEAT_FILE_PATTERN = '/data/tpj/tpj/S2VT/videocap-datasets/Youtube_senti/features/features_resnet_coco_pre/youtube_allframes_resnet_pool5_coco_pre_{0}.txt'
 
  #LSTM_NET_FILE = './s2vt.words_to_preds.deploy.prototxt'
  LSTM_NET_FILE = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream3/s2vt.words_to_preds.deploy.prototxt'
  #RESULTS_DIR = './results'
  RESULTS_DIR = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream3/results_82'
