...
def predict_single_word(net, pad_img_feature, previous_word, output1='probs', output2='probs_senti'):
  ...
  output_preds = 0.9*output_preds1 + 0.1*output_preds2       ###tpj
  return output_preds
...
def sample_captions(net, image_features,
  ...
  while num_done < batch_size:
    ...
    net_output_probs = 0.9*net_output_probs1 + 0.1*net_output_probs2           ###tpj
    for index in range(batch_size):
...
def main():
  ...
  #DIR = './snapshots'
  DIR = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream/models'
  ...
  LSTM_NET_FILE = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream/s2vt.words_to_preds.deploy.prototxt'
  #RESULTS_DIR = './results'
  RESULTS_DIR = '/data/tpj/tpj/S2VT/caffe-recurrent/examples/s2vt_senti/s2vt_test7_resnet_coco_pre_factor_pre_train_2stream/results2'
