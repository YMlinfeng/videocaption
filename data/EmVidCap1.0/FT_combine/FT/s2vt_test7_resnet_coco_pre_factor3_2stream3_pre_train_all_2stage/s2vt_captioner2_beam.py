...
#sys.path.insert(0,'/home/tpj/S2VT/caffe-recurrent/python')
CAFFE_PATH='/data/tpj/tpj/S2VT/caffe-recurrent/python/'
...
def predict_single_word(net, pad_img_feature, previous_word, output1='probs', output2='probs_senti'):
  cont_input = 1
  cont = np.array([cont_input])
  data_en = np.array([previous_word])
  ...
  net.forward(frames_fc7=image_features, cont_sentence=cont, input_sentence=data_en,
     stage_indicator=stage_ind)
  output_preds1 = net.blobs[output1].data.reshape(-1)                    ###tpj
  output_preds2 = net.blobs[output2].data.reshape(-1)                    ###tpj
  #output_preds3 = net.blobs[output3].data.reshape(-1)                    ###tpj
  output_preds = 0.5*output_preds1 + 0.5*output_preds2       ###tpj
  return output_preds

def predict_single_word_from_all_previous(net, video_feature, pad_img_feature, previous_words):
  encode_video_frames(net, video_feature)
...
def predict_image_caption(net, video_feature, pad_img_feature, vocab_list, strategy={'type': 'beam'}):
  assert 'type' in strategy
  assert strategy['type'] in ('beam', 'sample')
  if strategy['type'] == 'beam':
    return predict_image_caption_beam_search(net, video_feature, pad_img_feature, vocab_list, strategy)
...
def sample_image_caption(net, image, strategy, net_output1='predict', net_output2='predict_senti', max_length=50):
  sentence = []
  probs = []
  eps_prob = 1e-8
  ...
  while len(sentence) < max_length and (not sentence or sentence[-1] != 0):
    ...
    softmax_inputs = \
        predict_single_word(net, image, previous_word, output1=net_output1, output2=net_output2)
    word = random_choice_from_probs(softmax_inputs, temp)
    sentence.append(word)
    probs.append(softmax(softmax_inputs, 1.0)[word])
  return sentence, probs

def predict_image_caption_beam_search(net, video_feature, pad_img_feature, vocab_list, strategy, max_length=50):
  ...
  while beams_complete < len(beams):
    ...
    for beam_index, beam_log_prob, beam in \
      ...
      if beam_size == 1:
        ...
        probs = predict_single_word_from_all_previous(net, video_feature, pad_img_feature, beam)
...
def run_pred_iter(net, video_feature, pad_image_feature, vocab_list, strategies=[{'type': 'beam'}]):
  outputs = []
  for strategy in strategies:
    captions, probs = predict_image_caption(net, video_feature, pad_image_feature, vocab_list, strategy=strategy)
...
def run_pred_iters(pred_net, vidids, video_gt_pairs, fsg,
  ...
  for video_id in vidids:
    ...
    if descriptor_video_id != video_id:
      ...
      pad_img_feature = video_features[-1].copy()
      # Make padding all 0 when predicting
      pad_img_feature[pad_img_feature > 0] = 0
      desciptor_video_id = video_id
    outputs[video_id] = \
        run_pred_iter(pred_net, video_features, pad_img_feature, display_vocab, strategies=strategies)
...
def sample_captions(net, image_features,
    prob_output_name1='probs', prob_output_name2='probs_senti', output_name='samples', caption_source='sample'):
  ...
  while num_done < batch_size:
    ...
    net_output_probs1 = net.blobs[prob_output_name1].data                    ###tpj
    net_output_probs2 = net.blobs[prob_output_name2].data                    ###tpj
    #net_output_probs2 = net.blobs[prob_output_name3].data                    ###tpj
    net_output_probs = 0.5*net_output_probs1 + 0.5*net_output_probs2           ###tpj
    for index in range(batch_size):
...
def main():
  ...
  #DIR = './snapshots'
  DIR = '/data/tpj/Sentivideo_v2_20190910/FT_combine/s2vt_test7_resnet_coco_pre_factor3_2stream3_pre_train_all_2stage/models'
  #VOCAB_FILE = './yt_coco_mvad_mpiimd_vocabulary.txt'
  VOCAB_FILE = '/data/tpj/Sentivideo_v2_20190910/dataset_combine/vocabulary_threshold_1_combine_sorted.txt'
  #FRAMEFEAT_FILE_PATTERN = './yt_allframes_vgg_fc7_{0}.txt'
  FRAMEFEAT_FILE_PATTERN = '/data/tpj/Sentivideo_v2_20190910/FT_combine/features/features_resnet152/sentivideo_allframes_resnet152_pool5_coco_pre_{0}.txt'
 
  #LSTM_NET_FILE = './s2vt.words_to_preds.deploy.prototxt'
  LSTM_NET_FILE = '/data/tpj/Sentivideo_v2_20190910/FT_combine/s2vt_test7_resnet_coco_pre_factor3_2stream3_pre_train_all_2stage/s2vt.words_to_preds.deploy.prototxt'
  #RESULTS_DIR = './results'
  RESULTS_DIR = '/data/tpj/Sentivideo_v2_20190910/FT_combine/s2vt_test7_resnet_coco_pre_factor3_2stream3_pre_train_all_2stage/results_55'
