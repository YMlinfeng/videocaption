...
def predict_single_word(net, pad_img_feature, previous_word, output1='probs', output2='probs_senti'):
  ...
  output_preds = 1*output_preds1 + 0*output_preds2       ###tpj
  return output_preds
...
def sample_captions(net, image_features,
  ...
  while num_done < batch_size:
    ...
    net_output_probs = 1*net_output_probs1 + 0*net_output_probs2           ###tpj
    for index in range(batch_size):
...
def main():
  ...
  #RESULTS_DIR = './results'
  RESULTS_DIR = '/data/tpj/Sentivideo_v2_20190910/FT_combine/s2vt_test7_resnet_coco_pre_factor3_2stream3_pre_train_all_2stage/results_10'
